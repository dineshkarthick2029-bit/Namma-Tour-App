...
// ============================================================
// ROUTE PLANNER — uses offline-router.js (getRoute / renderRouteSVG)
// which itself needs routes-data.js loaded. Falls back to a plain
// message if either file wasn't bundled, so this never throws.
// ============================================================
function populateRouteSelects() {
  const fromSel = document.getElementById('routeFromSelect');
  const toSel = document.getElementById('routeToSelect');
  if (!fromSel || !toSel || typeof curatedPlaces === 'undefined') return;
  const optionsHtml = curatedPlaces
    .slice()
    .sort((a, b) => a.place.localeCompare(b.place))
    .map(p => `<option value="${p.id}">${escapeHtml(p.place)} (${escapeHtml(p.district)})</option>`)
    .join('');
  fromSel.innerHTML = optionsHtml;
  toSel.innerHTML = optionsHtml;
  // default to two different places so the first click shows something real
  if (curatedPlaces.length > 1) toSel.selectedIndex = 1;
}
...
document.getElementById('showRouteBtn')?.addEventListener('click', () => {
  ...
  if (!out) return;
  if (typeof renderRouteSVG !== 'function') {
    out.innerHTML = '<p class="helpHint">Route data not loaded — check routes-data.js and offline-router.js are included.</p>';
    return;
  }
  if (!fromId || !toId) return;
  if (fromId === toId) {
    out.innerHTML = '<p class="helpHint">Pick two different places.</p>';
    return;
  }
  renderRouteSVG('routeOutput', fromId, toId);
});
